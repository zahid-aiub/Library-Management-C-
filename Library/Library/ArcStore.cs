    ...
    public partial class ArcStore : Form
    {
        ...
        private void searchBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "select * from arc_book where book_name like('%" + textBox1.Text + "%')";
                cmd.ExecuteNonQuery();
        ...
        private void ArcStore_Load(object sender, EventArgs e)
        {
            if (uName == "" && uPass == "")
            {
        ...
        public void disp_books()
        {
            try
            {
                ...
                MySqlCommand cmd = con.CreateCommand();
                cmd.CommandType = CommandType.Text;
                cmd.CommandText = "select * from arc_book";
                cmd.ExecuteNonQuery();
                DataTable dt = new DataTable();
                MySqlDataAdapter da = new MySqlDataAdapter(cmd);
                da.Fill(dt);
        ...
        private void borrowBtn_Click(object sender, EventArgs e)
        {
            ...
            if (quantity_text.Text == x.ToString())
            {
                ...
                try
                {
                    MySqlCommand cmd = con.CreateCommand();
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "update arc_book set quantity='" + textBox2.Text + "'where book_name='" + b_name_text.Text + "'";
                    cmd.ExecuteNonQuery();
                    con.Close();
                    disp_books();
                }
