...
namespace Library
{
    public partial class EeeStore : Form
    {
        ...
        public EeeStore(string name, string pass)
        {
            this.uName = name;
            this.uPass = pass;
        ...
        private void searchBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "select * from eee_book where book_name like('%" + textBox1.Text + "%')";
                cmd.ExecuteNonQuery();
        ...
        private void EeeStore_Load(object sender, EventArgs e)
        {
            if (uName == "" && uPass == "")
            {
        ...
        private void borrowBtn_Click(object sender, EventArgs e)
        {
            ...
            if (quantity_text.Text == x.ToString())
            {
                ...
                try
                {
                    MySqlCommand cmd = con.CreateCommand();
                    cmd.CommandType = CommandType.Text;
                    cmd.CommandText = "update eee_book set quantity='" + textBox2.Text + "'where book_name='" + b_name_text.Text + "'";
                    cmd.ExecuteNonQuery();
                    con.Close();
                    disp_books();
                }
